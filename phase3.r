...
simulate_data <- function() {
    #Simulate Raw Data
    x <- rgamma(100000, 2, 3)
    x <- matrix(x, nrow = 1000, ncol = 100) #each row is a person, each column is a feature
    ...
    true_phi <- rep(0.005, 100) #simulate phi (true cutoff). If an element in x is lower than respective element in phi, then it is 0. If an element in x is higher than respective element in phi, then it is 1. 
...
screen_for_top_x <- function(screen) {
  # Returns indices of near-zero variance variables that are not y
  nzv <- nearZeroVar(screen[, names(screen) != "y"]) 

  # Drop those columns
  #if there are no features with zero variance, then keep it as the same screen
  #if there are features with zero variance, then drop them
  if (length(nzv) == 0) { 
  final_df_filtered <- screen
  } else {
  final_df_filtered <- screen[, -nzv]
  }

  # Perform a t test
  t_test <- function(feature){
    feature <- final_df_filtered %>% pull(!!sym(feature))
    y <- final_df_filtered %>% pull(!!sym("y"))
    return(t.test(feature ~ y)$p.value)
  }

  # Get a list of the feature names 
  feature_names <- names(final_df_filtered %>% select(-c("y")))

  # Pass in each feature name to the t test function --> returns a dataframe, where rows are species and one column denotes the p value
  feature_p_values <- as.data.frame(sapply(feature_names, t_test))
  colnames(feature_p_values) <- "p_value"

  # Return the top contestants (sorts p-value in descending order)
  contestants <- rownames(feature_p_values %>% arrange(p_value) %>% head(15))

  return(contestants)
}

logic_cv <- function(a, df, phi_test, folds_inner, num_contestants, trees, leaves) {
    ...
    }, as.data.frame(train_cv[,1:num_contestants]), phi_test)
    
    z_test <- mapply(function(z_col, phi_col) {
    # Apply z_to_x to each element of the column z_col with the corresponding phi
    sapply(z_col, function(z_element) x_to_z(z_element, phi_col))
    }, as.data.frame(test_cv[,1:num_contestants]), phi_test)
    
    capture.output(mod_greed <- logreg(
    train_cv$y,
    z_train,
    type = 3,
    select = 6, ntrees = trees, nleaves = leaves), file = NULL) #type = logic regression, select = greedy
    ...
    y_pred <- predict(mod_greed, newbin = z_test[,1:num_contestants])[[num_models]] #this returns the predicted values (probabilities) from the test data
...
pick_neighbor_to_j <- function(df, i,j, phis){
    ##PICKING A NEIGHBOR OF J
          ...
          diffs_filt <- diff[diff > 0 & diff <= 0.03] #find the differences within threshold of 0.03
...
sim_anneal_of_phi_vector_for_tree_leaf_combo_cv <- function(a, df, outer_folds, trees, leaves) { 
    training_screening <- df[-outer_folds[[a]],] #training folds
    hold_out <- df[outer_folds[[a]],] #test fold

    ## SPLIT TRAINING_SCREENING INTO TWO SEPARATE FOLDS
    set.seed(2024)
    training_screening_folds <- caret::createFolds(training_screening$y, k = 2) #creating folds
    set.seed(NULL)

    training <- training_screening[training_screening_folds$Fold1,]
    screening <- training_screening[training_screening_folds$Fold2,]

    ## CALL THE SCREEN FOR TOP X ON SCREENING PORTION
    filtered_coefs <- screen_for_top_x(screening)  #screening for top 15 features
    num_contestants <- length(filtered_coefs)
    train_filt <- training[, c(filtered_coefs,"y")] #only selecting columns in train (coefs) that made it into filtered coefs
    hold_out_filt <- hold_out[, c(filtered_coefs, "y")] #only selecting columns in hold out (coefs) that made it into filtered coefs
    ...
    phi_start <- unname(apply(train_filt[1:num_contestants], 2, make_phi_start))
    ...
    folds_inner <- caret::createFolds(train_filt$y, k = 10) #creating folds
    set.seed(NULL)
    ...
    while (switchy == TRUE) { #when probability gets smaller, or do like an epsilon check
      #add for loop for each temp settings run a bunch of iterations (allows more iterations at these larger temperatures and give you space to search) more iterations at the hotter temperature
        for (j in 1:num_contestants) { #for each element in phi 
        print(paste("i", i))
        print(paste("j", j)) #prints which iteration we are on 
        print(paste("temp",temp)) #prints the temperature we are on 

        phi_new <- pick_neighbor_to_j(train_filt, i, j, phis)

        log_loss_phi_new <- mean(unlist(mclapply(1:10, function(a) {logic_cv(a, train_filt, phi_new, folds_inner, num_contestants, trees, leaves)}, mc.cores = 10))) #test the current proposal phi 
        log_loss_phi_old <- mean(unlist(mclapply(1:10, function(a) {logic_cv(a, train_filt, phis[[i]], folds_inner, num_contestants, trees, leaves)}, mc.cores = 10))) #test the previous phi
      ...
      log_loss_post_j_updates <- mean(unlist(mclapply(1:10, function(a) {logic_cv(a, train_filt, phis[[i]], folds_inner, num_contestants, trees, leaves)}, mc.cores = 10))) #calculating the log loss after updating all j elements in phi 
      ...
      # Apply z_to_x to each element of the column z_col with the corresponding phi
      sapply(z_col, function(z_element) x_to_z(z_element, phi_col))
      }, as.data.frame(train_filt[,1:num_contestants]), final_phi)

      #fit a final logic regresison model on this dataframe
      ...
      # Apply z_to_x to each element of the column z_col with the corresponding phi
      sapply(z_col, function(z_element) x_to_z(z_element, phi_col))
      }, as.data.frame(hold_out_filt[,1:num_contestants]), final_phi)
      
      hold_out_preds <- predict(final_logic_model, newbin = z_hold_out_final_phi[,1:num_contestants])[[num_models]] #this returns the predicted values (probabilities) from the hold out set
      
      hold_out_truth <- hold_out_filt$y #real y 
...
sim_anneal_of_phi_vector_for_tree_leaf_combo_no_cv <- function(df, winner_tree, winner_leaf) {
      ## SPLIT TRAINING_SCREENING INTO TWO SEPARATE FOLDS
      set.seed(2024)
      training_screening_folds <- caret::createFolds(df$y, k = 2) #creating folds
      set.seed(NULL)

      training <- df[training_screening_folds$Fold1,]
      screening <- df[training_screening_folds$Fold2,]
      ...
      while (switchy == TRUE) { #when probability gets smaller, or do like an epsilon check
        #add for loop for each temp settings run a bunch of iterations (allows more iterations at these larger temperatures and give you space to search) more iterations at the hotter temperature
          ...
          log_loss_phi_new <- mean(unlist(mclapply(1:10, function(a) {logic_cv(a, train_filt, phi_new, folds_inner, num_contestants, winner_tree, winner_leaf)}, mc.cores = 10))) #test the current proposal phi 
          log_loss_phi_old <- mean(unlist(mclapply(1:10, function(a) {logic_cv(a, train_filt, phis[[i]], folds_inner, num_contestants, winner_tree, winner_leaf)}, mc.cores = 10))) #test the previous phi
        ...
        log_loss_post_j_updates <- mean(unlist(mclapply(1:10, function(a) {logic_cv(a, train_filt, phis[[i]], folds_inner, num_contestants, winner_tree, winner_leaf)}, mc.cores = 10))) #calculating the log loss after updating all j elements in phi 
      ...
      #binarize the training fold with that final phi vector
      z_train_final_phi <- mapply(function(z_col, phi_col) {
      # Apply z_to_x to each element of the column z_col with the corresponding phi
      sapply(z_col, function(z_element) x_to_z(z_element, phi_col))
      }, as.data.frame(train_filt[,1:num_contestants]), final_phi)
      ...
      z_train_final_phi,
      type = 3,
      select = 6, ntrees = winner_tree, nleaves = winner_leaf), file = NULL) #type = logic regression, select = greedy
      ...
      return(list(winner_tree, winner_leaf, logic_expr, final_phi, filtered_coefs)) #log loss on particular fold
    } 

#OUTPUT OF THIS IS A LIST OF T/L AND THEIR CV LOSSES
run_tree_leaf_combo <- function(tl_row, df){
  set.seed(2024) #setting seed so we have the same folds every time across each of the tree/leaf combinations
  outer_folds <- caret::createFolds(df$y, k = 5) #creating folds
  set.seed(NULL)
  ...
  output <- mclapply(1:5, function (a) {sim_anneal_of_phi_vector_for_tree_leaf_combo_cv(a, df, outer_folds, tree, leaf)}, mc.cores = 5) 

  results_list <- list(tree, leaf, output[1], output[2], output[3], output[4], output[5])
...
colnames(results_list_per_run_df) <- c("Tree", "Leaf", "HoldOut1", "LE1", "HoldOut2", "LE2", "HoldOut3", "LE3", "HoldOut4", "LE4", "HoldOut5", "LE5")
...
results_list_per_run_df$HoldOut3 <- as.numeric(results_list_per_run_df$HoldOut3)
results_list_per_run_df$HoldOut4 <- as.numeric(results_list_per_run_df$HoldOut4)
results_list_per_run_df$HoldOut5 <- as.numeric(results_list_per_run_df$HoldOut5)
...
#SAVING RESULTS
saveRDS(results_list_per_run_df, sprintf("/home/shanbhagn123/phase3_screen_15/MS_evals_phase3/find_MS_%03d.rds", dataset_id))
saveRDS(final_logic_exp_and_phi, sprintf("/home/shanbhagn123/phase3_screen_15/winner_LEs_phase3/winner_LE_%03d.rds", dataset_id))
end.time <- Sys.time()
